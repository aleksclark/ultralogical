// Package envprovider is the provider seam's registry: it maps a provider
...
func (r *Registry) Register(kind string, factory Factory) {
	...
	if _, exists := r.factories[kind]; exists {
		panic("envprovider: provider kind " + kind + " is registered twice")
	}
...
func (r *Registry) Build(ctx context.Context, kind string, config json.RawMessage) (uc.EnvProvider, error) {
	...
	r.mu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("envprovider: provider kind %q is not enabled in this deployment", kind)
	}
