...
func encodeHandle(d handleData) (uc.ProviderHandle, error) {
	...
	if err != nil {
		return uc.ProviderHandle{}, fmt.Errorf("k8s: encode handle: %w", err)
	}
...
func (p *Provider) applySecret(ctx context.Context, namespace, name string, envID uc.EnvID, token string) error {
...
func (p *Provider) applyPod(ctx context.Context, namespace, name string, envID uc.EnvID, spec uc.EnvSpec) error {
...
func (p *Provider) applyService(ctx context.Context, namespace, name string, envID uc.EnvID) (*corev1.Service, error) {
...
func (p *Provider) Status(ctx context.Context, handle uc.ProviderHandle) (uc.ProviderStatus, error) {
	...
	if apierrors.IsNotFound(err) {
		return uc.ProviderStatus{State: uc.EnvFailed, Message: "pod not found"}, nil
	}
	if err != nil {
		return uc.ProviderStatus{}, fmt.Errorf("k8s: get pod: %w", err)
	}
	...
	case corev1.PodRunning:
		...
		return uc.ProviderStatus{State: uc.EnvProvisioning, Message: "pod is not ready"}, nil
	case corev1.PodPending:
		return uc.ProviderStatus{State: uc.EnvProvisioning, Message: podPendingReason(pod)}, nil
	case corev1.PodSucceeded, corev1.PodFailed:
		return uc.ProviderStatus{State: uc.EnvFailed, Message: string(pod.Status.Phase)}, nil
	default:
		return uc.ProviderStatus{State: uc.EnvProvisioning}, nil
	}
}
...
func (p *Provider) Restart(ctx context.Context, envID uc.EnvID, handle uc.ProviderHandle, spec uc.EnvSpec, token string) (uc.ProviderHandle, error) {
	...
	if err != nil && !apierrors.IsNotFound(err) {
		return uc.ProviderHandle{}, fmt.Errorf("k8s: delete pod for restart: %w", err)
	}
...
// Adopt implements uc.EnvAdopter: it finds resources this provider already
...
func (p *Provider) Adopt(ctx context.Context, envID uc.EnvID) (uc.ProviderHandle, bool, error) {
	...
	if err != nil {
		...
		return uc.ProviderHandle{}, false, fmt.Errorf("k8s: adopt lookup: %w", err)
	}
	if len(pods.Items) == 0 {
		return uc.ProviderHandle{}, false, nil
	}
...
// Resources implements uc.EnvResourceLister by enumerating the live
