...
// documented walkthrough is executable rather than aspirational. It drives no
// remote control plane and is not a deployment target.
package static
...
func (p *Provider) Status(_ context.Context, handle ultra.ProviderHandle) (ultra.ProviderStatus, error) {
	...
	if err := syscall.Kill(d.PID, 0); err != nil {
		return ultra.ProviderStatus{State: ultra.EnvFailed, Message: "sandbox process is gone"}, nil
	}
...
func (p *Provider) Probe(context.Context) (ultra.ProviderCapabilities, error) {
	capabilities := ultra.ProviderCapabilities{Kind: "static", Notes: map[ultra.ProviderCapability]string{
