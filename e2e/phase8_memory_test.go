...
// A8.6 — memory and presence across concurrency, replay, expiry, and tenancy.
func TestA86_MemoryAndPresence(t *testing.T) {
	stack := harness.Up(t, harness.WithReplicas(2, 2))
	ctx := context.Background()
	org := stack.OrgA.ID
	alice := stack.IngressClient(harness.TokenAlice)
	bob := stack.BobClient()
	sess := createSession(t, alice, string(org), "memory and presence")
	...
	t.Run("concurrent_writes_are_atomic", func(t *testing.T) {
		...
		for i := range writers {
			...
			go func(i int) {
				defer wg.Done()
				_, err := alice.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
		...
		got, err := alice.Sessions.GetMemory(ctx, connect.NewRequest(&ultrav1.GetMemoryRequest{
			SessionId: sess.GetId(), Key: "contended.key",
		}))
		if err != nil {
			t.Fatal(err)
		...
		events := collectEvents(t, stack, sess.GetId(), ultra.EventKindMemorySet, 60*time.Second, wantEvents)
		setsForKey := 0
	...
	t.Run("key_namespace_is_enforced", func(t *testing.T) {
		...
		for _, key := range []string{"investigation.findings.db", "plain", "a-b_c.9"} {
			if _, err := alice.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
	...
	t.Run("caps_are_enforced", func(t *testing.T) {
		...
		big := `"` + strings.Repeat("x", ultra.MaxMemoryValue+1) + `"`
		if _, err := alice.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
		...
		for i := range ultra.MaxMemoryKeys {
			if _, err := alice.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
				SessionId: capped.GetId(), Key: fmt.Sprintf("filler.%03d", i), ValueJson: "true",
			})); err != nil {
				t.Fatalf("key %d of the allowed %d was rejected: %v", i, ultra.MaxMemoryKeys, err)
			}
		}
		if _, err := alice.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
			SessionId: capped.GetId(), Key: "one.too.many", ValueJson: "true",
		})); err == nil {
			t.Fatalf("key %d was accepted past the documented cap", ultra.MaxMemoryKeys+1)
		}
	...
	t.Run("agent_written_memory_survives_run_boundaries", func(t *testing.T) {
		...
		alice.AwaitRunState(t, writer.GetId(), ultrav1.RunState_RUN_STATE_COMPLETED, 90*time.Second)

		reader, _, err := alice.StartRun(ctx, shared.GetId(), "reader run")
		if err != nil {
			t.Fatal(err)
		}
		alice.AwaitRunState(t, reader.GetId(), ultrav1.RunState_RUN_STATE_COMPLETED, 90*time.Second)

		// The second run really read the value the first wrote.
		results := toolResultsFor(t, stack, shared.GetId(), ultra.RunID(reader.GetId()))
		var readBack bool
		...
		if got.Msg.GetEntry().GetUpdatedByType() != string(ultra.ActorAgent) {
			t.Fatalf("memory attributed to %q, want the agent that wrote it", got.Msg.GetEntry().GetUpdatedByType())
		}
	})

	t.Run("presence_expires_and_replays_identically", func(t *testing.T) {
		// Presence must reconstruct from the log, and must expire on its own
		// when a participant stops heartbeating.
		room := createSession(t, alice, string(org), "presence room")
		if _, err := alice.Sessions.Join(ctx, connect.NewRequest(&ultrav1.JoinRequest{
			SessionId: room.GetId(), Display: "Alice",
		})); err != nil {
			t.Fatal(err)
		}

		// Active immediately after joining.
		list, err := alice.Sessions.ListParticipants(ctx, connect.NewRequest(&ultrav1.ListParticipantsRequest{
			SessionId: room.GetId(),
		}))
		if err != nil {
			t.Fatal(err)
		}
		if len(list.Msg.GetParticipants()) != 1 || list.Msg.GetParticipants()[0].GetState() != string(ultra.PresenceActive) {
			t.Fatalf("participant is not active after joining: %+v", list.Msg.GetParticipants())
		}

		// Without heartbeats the reaper expires them to idle. The harness runs
		// with a short threshold so this is observable.
		...
		for time.Now().Before(deadline) {
			...
			if len(list.Msg.GetParticipants()) == 1 && list.Msg.GetParticipants()[0].GetState() == string(ultra.PresenceIdle) {
				idle = true
				break
			}
		...
		if !idle {
			t.Fatal("presence never expired; a vanished participant would appear present forever")
		}
		// The transition is in the log, so subscribers learn about it too.
		collectEventsIn(t, stack, room.GetId(), ultra.EventKindParticipantIdle, 60*time.Second, 1)

		// Replay reconstructs the same presence story from seq 0.
		sub, err := alice.Subscribe(ctx, room.GetId(), 0)
		if err != nil {
			t.Fatal(err)
		}
		defer sub.Close()
		replayed := sub.CollectUntil(t, 60*time.Second, func(ev *ultrav1.SessionEvent) bool {
			return testclient.Kind(ev) == "participant_idle"
		})
		var sawJoin, sawIdle bool
		...
		if !sawJoin || !sawIdle {
			t.Fatalf("replay did not reconstruct presence transitions (join=%v idle=%v)", sawJoin, sawIdle)
		}
	})

	t.Run("cross_tenant_access_is_indistinguishable", func(t *testing.T) {
		// Bob is in another org. Reading Alice's memory and presence must look
		...
		_, foreignMem := bob.Sessions.GetMemory(ctx, connect.NewRequest(&ultrav1.GetMemoryRequest{
			SessionId: sess.GetId(), Key: "tenant.secret",
		}))
		_, missingMem := bob.Sessions.GetMemory(ctx, connect.NewRequest(&ultrav1.GetMemoryRequest{
			SessionId: missing, Key: "tenant.secret",
		}))
		assertSameDenial(t, "GetMemory", foreignMem, missingMem)

		_, foreignList := bob.Sessions.ListMemory(ctx, connect.NewRequest(&ultrav1.ListMemoryRequest{SessionId: sess.GetId()}))
		_, missingList := bob.Sessions.ListMemory(ctx, connect.NewRequest(&ultrav1.ListMemoryRequest{SessionId: missing}))
		assertSameDenial(t, "ListMemory", foreignList, missingList)

		_, foreignPresence := bob.Sessions.ListParticipants(ctx, connect.NewRequest(&ultrav1.ListParticipantsRequest{SessionId: sess.GetId()}))
		_, missingPresence := bob.Sessions.ListParticipants(ctx, connect.NewRequest(&ultrav1.ListParticipantsRequest{SessionId: missing}))
		assertSameDenial(t, "ListParticipants", foreignPresence, missingPresence)

		_, foreignWrite := bob.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
			SessionId: sess.GetId(), Key: "tenant.intrusion", ValueJson: "true",
		}))
		_, missingWrite := bob.Sessions.SetMemory(ctx, connect.NewRequest(&ultrav1.SetMemoryRequest{
		...
		if _, err := alice.Sessions.GetMemory(ctx, connect.NewRequest(&ultrav1.GetMemoryRequest{
...
func collectEventsIn(t *testing.T, stack *harness.Stack, session, kind string, timeout time.Duration, n int) []ultra.Event {
	t.Helper()
