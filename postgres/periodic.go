...
func (s *periodicStore) Create(ctx context.Context, p ultra.PeriodicPrompt) error {
...
func scanPeriodic(row pgx.Row) (ultra.PeriodicPrompt, error) {
	var p ultra.PeriodicPrompt
	var schedule string
	var runID *string
	err := row.Scan(&p.ID, &p.OrgID, &p.SessionID, &runID, &schedule, &p.Prompt, &p.Enabled, &p.NextAt, &p.CreatedAt)
	if runID != nil {
		p.RunID = ultra.RunID(*runID)
	}
	if errors.Is(err, pgx.ErrNoRows) {
		return p, ultra.ErrNotFound
	}
	p.Schedule, _ = time.ParseDuration(schedule)
	return p, err
}
func (s *periodicStore) List(ctx context.Context, session ultra.SessionID) ([]ultra.PeriodicPrompt, error) {
...
func (s *periodicStore) GetForUpdate(ctx context.Context, id ultra.PeriodicPromptID) (ultra.PeriodicPrompt, error) {
...
func (s *periodicStore) SetEnabled(ctx context.Context, id ultra.PeriodicPromptID, v bool) error {
	_, err := s.scope.s.db().Exec(ctx, `UPDATE periodic_prompts SET enabled=$3 WHERE id=$1 AND org_id=$2`, string(id), string(s.scope.org), v)
	return err
}
func (s *periodicStore) SetNext(ctx context.Context, id ultra.PeriodicPromptID, next time.Time) error {
