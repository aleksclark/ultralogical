...
func scanEnv(row pgx.Row) (uc.DevEnv, error) {
	var env uc.DevEnv
	...
	err := row.Scan(&env.ID, &env.OrgID, &env.SessionID, &env.ProviderInstanceID,
		...
		&env.CreatedAt, &env.UpdatedAt, &env.ReadyAt, &env.TerminatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return uc.DevEnv{}, uc.ErrNotFound
	}
	if err != nil {
		return uc.DevEnv{}, fmt.Errorf("postgres: scan env: %w", err)
	}
	if err := json.Unmarshal(spec, &env.Spec); err != nil {
		return uc.DevEnv{}, fmt.Errorf("postgres: decode env spec: %w", err)
	}
	if err := json.Unmarshal(handle, &env.Handle); err != nil {
		return uc.DevEnv{}, fmt.Errorf("postgres: decode provider handle: %w", err)
	}
	return env, nil
}

func (s *envStore) Create(ctx context.Context, env uc.DevEnv) error {
...
func (s *envStore) Get(ctx context.Context, id uc.EnvID) (uc.DevEnv, error) {
...
func (s *envStore) GetForUpdate(ctx context.Context, id uc.EnvID) (uc.DevEnv, error) {
	return scanEnv(s.scope.s.db().QueryRow(ctx,
		`SELECT `+envColumns+` FROM dev_envs WHERE id = $1 AND org_id = $2 FOR UPDATE`,
		string(id), string(s.scope.org)))
}

func (s *envStore) List(ctx context.Context, session uc.SessionID) ([]uc.DevEnv, error) {
...
func (s *envStore) ListActive(ctx context.Context) ([]uc.DevEnv, error) {
	return s.list(ctx, `SELECT `+envColumns+` FROM dev_envs
		WHERE org_id = $1 AND state NOT IN ('terminated', 'failed') ORDER BY created_at`, string(s.scope.org))
}

func (s *envStore) list(ctx context.Context, sql string, args ...any) ([]uc.DevEnv, error) {
...
func (s *envStore) update(ctx context.Context, id uc.EnvID, sql string, args ...any) error {
...
func (s *envStore) SetProvisioning(ctx context.Context, id uc.EnvID) error {
	return s.update(ctx, id, `UPDATE dev_envs SET state='provisioning', updated_at=now()
		WHERE id=$1 AND org_id=$2`)
}

func (s *envStore) SetHandle(ctx context.Context, id uc.EnvID, handle uc.ProviderHandle) error {
...
func (s *envStore) SetReady(ctx context.Context, id uc.EnvID, handle uc.ProviderHandle, endpoint string) error {
	h, err := json.Marshal(handle)
	if err != nil {
		return err
	}
...
func (s *envStore) SetFailed(ctx context.Context, id uc.EnvID, message string) error {
...
func (s *envStore) SetSuspended(ctx context.Context, id uc.EnvID, message string) error {
	tag, err := s.scope.s.db().Exec(ctx,
...
func (s *envStore) SetTerminating(ctx context.Context, id uc.EnvID) error {
	return s.update(ctx, id, `UPDATE dev_envs SET state='terminating', updated_at=now()
		WHERE id=$1 AND org_id=$2`)
}

func (s *envStore) SetTerminated(ctx context.Context, id uc.EnvID) error {
	return s.update(ctx, id, `UPDATE dev_envs SET state='terminated',
		terminated_at=now(), updated_at=now() WHERE id=$1 AND org_id=$2`)
}

func (s *envStore) RotateToken(ctx context.Context, id uc.EnvID, hash, enc []byte) error {
