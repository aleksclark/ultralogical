...
// Provider implements uc.ResourceProvider on Nomad.
...
func jobID(envID uc.ResourceID) string { return "ultra-env-" + sanitize(string(envID)) }
...
func (p *Provider) ValidateSpec(spec json.RawMessage) error {
	...
	if s.Name == "" {
		return fmt.Errorf("nomad: spec.name is required")
	}
	return nil
}
...
func (p *Provider) HealthCheck(ctx context.Context, r uc.Resource) error {
	...
	if st.State != uc.ResourceReady {
		if st.Message != "" {
			return fmt.Errorf("nomad: not ready: %s", st.Message)
		}
		return fmt.Errorf("nomad: not ready: %s", st.State)
	}
	return nil
}
...
func (p *Provider) Provision(ctx context.Context, r uc.Resource, token string) (json.RawMessage, error) {
	...
	if _, _, err := p.client.Jobs().Register(job, p.write(ctx)); err != nil {
		return nil, fmt.Errorf("nomad: register job: %w", err)
	}
	return encodeHandle(handleData{JobID: id, ResourceID: string(envID)})
}
...
func (p *Provider) Status(ctx context.Context, r uc.Resource) (uc.ResourceStatus, error) {
	...
	if err != nil {
		if isNotFound(err) {
			return uc.ResourceStatus{State: uc.ResourceFailed, Message: "job not found"}, nil
		}
		return uc.ResourceStatus{}, fmt.Errorf("nomad: job info: %w", err)
	}
	if job.Stop != nil && *job.Stop {
		return uc.ResourceStatus{State: uc.ResourceFailed, Message: "job is stopped"}, nil
	}
	allocations, _, err := p.client.Jobs().Allocations(d.JobID, true, p.query(ctx))
	if err != nil {
		return uc.ResourceStatus{}, fmt.Errorf("nomad: job allocations: %w", err)
	}
	if len(allocations) == 0 {
		return uc.ResourceStatus{State: uc.ResourceProvisioning, Message: "no allocation yet"}, nil
	}
	for _, allocation := range allocations {
		...
		case "pending":
			return uc.ResourceStatus{State: uc.ResourceProvisioning, Message: "allocation pending"}, nil
		}
	}
	return uc.ResourceStatus{State: uc.ResourceFailed, Message: allocations[0].ClientStatus}, nil
}
...
func (p *Provider) Restart(ctx context.Context, r uc.Resource, token string) (json.RawMessage, error) {
	if err := p.Terminate(ctx, r); err != nil {
		return nil, err
	}
	if err := p.awaitJobAbsent(ctx, jobID(r.ID)); err != nil {
		return nil, err
	}
...
// Adopt implements uc.ResourceAdopter: it finds a job this provider already
...
func (p *Provider) Adopt(ctx context.Context, r uc.Resource) (json.RawMessage, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("nomad: adopt lookup: %w", err)
	}
	if job.Stop != nil && *job.Stop {
		return nil, false, nil
	}
	handle, err := encodeHandle(handleData{JobID: id, ResourceID: string(envID)})
	return handle, err == nil, err
}

// Resources implements uc.ResourceLister by enumerating the live Nomad
// objects this provider owns for an environment.
func (p *Provider) ListOwned(ctx context.Context) ([]uc.OwnedResource, error) {
	jobs, _, err := p.client.Jobs().List(p.query(ctx))
	if err != nil {
		return nil, fmt.Errorf("nomad: list jobs: %w", err)
	}
	var out []uc.OwnedResource
	for _, j := range jobs {
		if j == nil || !strings.HasPrefix(j.ID, "ultracore-") {
			continue
		}
		id := uc.ResourceID(strings.TrimPrefix(j.ID, "ultracore-"))
...
func (p *Provider) Probe(ctx context.Context) (uc.ProviderCapabilities, error) {
	...
	capabilities.Notes[uc.CapabilityRestartPreservesState] =
