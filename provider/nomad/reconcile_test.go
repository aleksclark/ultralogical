...
func TestA104_NomadReconcilesExternallyStoppedAllocation(t *testing.T) {
	...
	converged := harness.Await(t, env.ID, uc.ResourceFailed, 2*time.Minute)
...
func liveJobsFor(t *testing.T, client *nomadapi.Client, envID uc.ResourceID) int {
	t.Helper()
	...
	for _, stub := range listed {
		...
		if job.Meta["ultracore.resource_id"] == string(envID) {
			live++
		}
	}
	return live
...
func jobIDFor(envID uc.ResourceID) string {
...
func runningAllocation(t *testing.T, ctx context.Context, client *nomadapi.Client, envID uc.ResourceID) *nomadapi.Allocation {
	t.Helper()
...
func TestA104_AllocationCarriesDeclaredResources(t *testing.T) {
	...
	handle, err := provider.Provision(ctx, testRes(envID, uc.DevEnvSpec{Name: "resources", Workdir: "/work"}), "token")
	if err != nil {
		t.Fatal(err)
	}
