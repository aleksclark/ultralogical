...
func (a *Agent) provision(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	res := uc.Resource{ID: req.ResourceID, Kind: uc.ResourceKindDevEnv}
	if b, err := json.Marshal(req.Spec); err == nil {
		res.Spec = b
	}
	handle, err := a.Provider.Provision(r.Context(), res, req.Token)
	if err != nil {
...
func (a *Agent) status(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	status, err := a.Provider.Status(r.Context(), uc.Resource{Handle: req.Handle})
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
...
func (a *Agent) endpoint(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	local, err := a.Provider.Endpoint(r.Context(), uc.Resource{Handle: req.Handle})
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
	...
	writeJSON(w, EndpointResponse{Endpoint: ep})
}
...
func (a *Agent) restart(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	res := uc.Resource{ID: req.ResourceID, Kind: uc.ResourceKindDevEnv, Handle: req.Handle}
	if b, err := json.Marshal(req.Spec); err == nil {
		res.Spec = b
	}
	handle, err := a.Provider.Restart(r.Context(), res, req.Token)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
...
func (a *Agent) terminate(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	if err := a.Provider.Terminate(r.Context(), uc.Resource{Handle: req.Handle}); err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
	writeJSON(w, struct{}{})
...
func (a *Agent) resources(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	lister, ok := a.Provider.(uc.ResourceLister)
	if !ok {
		writeJSON(w, ResourcesResponse{})
		return
	}
	owned, err := lister.ListOwned(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadGateway)
		return
	}
	...
	for _, o := range owned {
		if o.ResourceID == req.ResourceID {
			resources = o.Descriptors
			break
		}
	}
...
func (a *Agent) revoke(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	a.Revoke()
	if len(req.Handle) > 0 && string(req.Handle) != "null" {
		_ = a.Provider.Terminate(r.Context(), uc.Resource{Handle: req.Handle})
	}
	writeJSON(w, struct{}{})
}
