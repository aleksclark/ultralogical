...
// Provider implements uc.ResourceProvider on Kubernetes.
...
func objectName(envID uc.ResourceID) string { return "ultra-env-" + sanitize(string(envID)) }
...
func (p *Provider) selector(envID uc.ResourceID) string {
	return fmt.Sprintf("%s=%s,%s=%s", LabelResourceID, sanitize(string(envID)), LabelManagedBy, ManagedByValue)
}
