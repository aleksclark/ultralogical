...
// reconciliation lives in resourcework, so an adapter that reported the deletion
...
func TestA102_KubernetesReconcilesExternallyDeletedPod(t *testing.T) {
	...
	harness := resourceconverge.New(t, provider, resourceconverge.Options{
	...
	ready := harness.Await(t, env.ID, uc.ResourceReady, 3*time.Minute)
	...
	converged := harness.Await(t, env.ID, uc.ResourceFailed, 90*time.Second)
	...
	live, err := client.CoreV1().Pods(reconcileNamespace).List(ctx, metav1.ListOptions{
		LabelSelector: k8s.LabelResourceID + "=" + sanitizedResourceID(env.ID),
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestA102_KubernetesAdoptsInterruptedProvisioning(t *testing.T) {
	...
	env := harness.Request(t, uc.DevEnvSpec{Name: "adopt", Workdir: "/work"})
	...
	orphan, err := provider.Provision(ctx, uc.Resource{ID: env.ID, Kind: uc.ResourceKindDevEnv, Spec: env.Spec}, harness.ClearToken(t, env))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = provider.Terminate(context.Background(), uc.Resource{Handle: orphan}) })
	created := podNameFor(t, ctx, client, env.ID)

	harness.Start(t)
	harness.Await(t, env.ID, uc.ResourceReady, 3*time.Minute)
...
func podNameFor(t *testing.T, ctx context.Context, client kubernetes.Interface, envID uc.ResourceID) string {
	t.Helper()
	pods, err := client.CoreV1().Pods(reconcileNamespace).List(ctx, metav1.ListOptions{
		LabelSelector: k8s.LabelResourceID + "=" + sanitizedResourceID(envID),
	})
	if err != nil {
		t.Fatal(err)
	}
...
// sanitizedResourceID mirrors the label value the adapter writes. A UUID is
// already a valid label value, so this only guards against a future id shape
// that is not.
func sanitizedResourceID(id uc.ResourceID) string { return string(id) }
