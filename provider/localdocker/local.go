// Package localdocker implements uc.ResourceProvider with Docker containers.
...
const labelResourceID = "ultracore.resource_id"
...
// Provider implements uc.ResourceProvider.
...
func volumeName(id uc.ResourceID) string { return "ultracore-env-" + string(id) }

func encodeHandle(h handleData) (json.RawMessage, error) {
	return handlefmt.EncodeHandle(1, h)
}
...
func (p *Provider) ValidateSpec(spec json.RawMessage) error {
	...
	if s.Name == "" {
		return fmt.Errorf("localdocker: spec.name is required")
	}
	return nil
}
...
func (p *Provider) HealthCheck(ctx context.Context, r uc.Resource) error {
	...
	if st.State != uc.ResourceReady {
		if st.Message != "" {
			return fmt.Errorf("localdocker: not ready: %s", st.Message)
		}
		return fmt.Errorf("localdocker: not ready: %s", st.State)
	}
	return nil
}

// Provision implements uc.ResourceProvider.
func (p *Provider) Provision(ctx context.Context, r uc.Resource, token string) (json.RawMessage, error) {
	...
	resp, err := p.docker.ContainerCreate(ctx,
		&containertypes.Config{
			...
			Labels:       map[string]string{labelResourceID: string(envID)},
	...
	if err != nil {
		return nil, fmt.Errorf("localdocker: create: %w", err)
	}
	if err := p.docker.ContainerStart(ctx, resp.ID, containertypes.StartOptions{}); err != nil {
		...
		if !strings.Contains(err.Error(), "address already in use") {
			return nil, fmt.Errorf("localdocker: start: %w", err)
		}
		_ = p.docker.ContainerRemove(ctx, resp.ID, containertypes.RemoveOptions{Force: true})
		return p.provisionRetry(ctx, r, token)
	}
...
func (p *Provider) provisionRetry(ctx context.Context, r uc.Resource, token string) (json.RawMessage, error) {
	attempt, _ := ctx.Value(provisionAttemptKey{}).(int)
	if attempt >= 5 {
		return nil, errors.New("localdocker: every published port this host offered was already in use")
	}
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	...
	return p.Provision(context.WithValue(ctx, provisionAttemptKey{}, attempt+1), r, token)
}
...
func (p *Provider) Status(ctx context.Context, r uc.Resource) (uc.ResourceStatus, error) {
	...
	return uc.ResourceStatus{State: uc.ResourceFailed, Message: info.State.Status}, nil
}

// Endpoint implements uc.ResourceProvider.
func (p *Provider) Endpoint(_ context.Context, r uc.Resource) (uc.ToolEndpoint, error) {
	d, err := decodeHandle(r.Handle)
	if err != nil {
		return "", err
	}
	return uc.ToolEndpoint(fmt.Sprintf("http://127.0.0.1:%d/mcp", d.HostPort)), nil
}
...
// Terminate implements uc.ResourceProvider. It removes both container and
...
func ContainerID(handle json.RawMessage) (string, error) {
	d, err := decodeHandle(handle)
	return d.ContainerID, err
}

// Adopt implements uc.ResourceAdopter. Containers are labelled and named by
...
func (p *Provider) Adopt(ctx context.Context, r uc.Resource) (json.RawMessage, bool, error) {
	...
	list, err := p.docker.ContainerList(ctx, containertypes.ListOptions{
		All:     true,
		Filters: filters.NewArgs(filters.Arg("label", labelResourceID+"="+string(id))),
	})
	if err != nil {
		return nil, false, fmt.Errorf("localdocker: adopt list: %w", err)
	}
	if len(list) == 0 {
	...
	if err != nil {
		return nil, false, fmt.Errorf("localdocker: adopt inspect: %w", err)
	}
	if !info.State.Running {
		if err := p.docker.ContainerStart(ctx, list[0].ID, containertypes.StartOptions{}); err != nil {
			return nil, false, fmt.Errorf("localdocker: adopt start: %w", err)
		}
	}
...
// ListOwned implements uc.ResourceLister.
func (p *Provider) ListOwned(ctx context.Context) ([]uc.OwnedResource, error) {
	containers, err := p.docker.ContainerList(ctx, containertypes.ListOptions{
		All:     true,
		Filters: filters.NewArgs(filters.Arg("label", labelResourceID)),
	})
	if err != nil {
		return nil, fmt.Errorf("localdocker: list containers: %w", err)
	}
	byID := map[uc.ResourceID]*uc.OwnedResource{}
	order := []uc.ResourceID{}
	for _, c := range containers {
		id := uc.ResourceID(c.Labels[labelResourceID])
		if id == "" {
			continue
		}
		owned, ok := byID[id]
		if !ok {
			owned = &uc.OwnedResource{ResourceID: id}
			byID[id] = owned
			order = append(order, id)
		}
		owned.Descriptors = append(owned.Descriptors, "container:"+c.ID)
	}
	for id, owned := range byID {
		name := volumeName(id)
		if _, err := p.docker.VolumeInspect(ctx, name); err == nil {
			owned.Descriptors = append(owned.Descriptors, "volume:"+name)
		} else if !cerrdefs.IsNotFound(err) {
			return nil, fmt.Errorf("localdocker: inspect volume: %w", err)
		}
	}
	out := make([]uc.OwnedResource, 0, len(order))
...
// Resources returns descriptors for one resource id (test helper).
func (p *Provider) Resources(ctx context.Context, id uc.ResourceID) ([]string, error) {
	var out []string
	containers, err := p.docker.ContainerList(ctx, containertypes.ListOptions{
		All:     true,
		Filters: filters.NewArgs(filters.Arg("label", labelResourceID+"="+string(id))),
...
// KillByResourceID kills the provider resource, used by reconciliation tests.
func (p *Provider) KillByResourceID(ctx context.Context, id uc.ResourceID) error {
	list, err := p.docker.ContainerList(ctx, containertypes.ListOptions{All: true, Filters: filters.NewArgs(filters.Arg("label", labelResourceID+"="+string(id)))})
	if err != nil {
		return err
	}
	if len(list) == 0 {
