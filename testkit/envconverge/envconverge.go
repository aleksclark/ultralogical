// Package envconverge drives the real durable environment lifecycle against
// one provider adapter, so a test can prove what happens when something
// outside the platform destroys a resource.
//
// Reconciliation lives in envwork, not in the adapters, so asserting an
...
func New(t *testing.T, provider uc.EnvProvider, options Options) *Harness {
	...
	h := &Harness{Store: store, Envs: envs, Keyring: keyring, queue: queue, pool: pool}
