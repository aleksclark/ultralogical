...
// documentedCommands extracts the `core ...` invocations a guide tells a
...
func documentedCommands(t *testing.T, guide string) []string {
	...
	for _, line := range strings.Split(string(body), "\n") {
		...
		if inBlock && strings.HasPrefix(strings.TrimSpace(line), "core ") {
			out = append(out, strings.TrimSpace(line))
		}
	}
	if len(out) == 0 {
		t.Fatalf("%s documents no core commands", guide)
	}
	return out
}

// A10.9 — the onboarding guide is executed, not merely written. Every `core`
...
func TestA109_KubernetesOnboardingGuideIsExecutable(t *testing.T) {
	...
	config, err := json.Marshal(map[string]any{
		"kubeconfig": kubeconfig, "namespace": "ultracore-envs",
	...
	for _, documented := range commands {
		...
		if strings.HasPrefix(documented, "core provider register") {
			if !strings.Contains(stdout, "byo-cluster") {
				t.Fatalf("registration did not report the provider it created:\n%s", stdout)
			}
		}
		if strings.HasPrefix(documented, "core provider list") {
			var listed struct {
				Providers []struct {
...
func onboardingArgs(t *testing.T, documented, config string) []string {
	t.Helper()
	switch {
	case strings.HasPrefix(documented, "core provider register"):
		return []string{"provider", "register", "byo-cluster", "--kind", "byo_k8s", "--config", config}
	case strings.HasPrefix(documented, "core provider list"):
		return []string{"provider", "list", "--json"}
	case strings.HasPrefix(documented, "core provider show"):
		return []string{"provider", "show", "byo-cluster", "--json"}
	case strings.HasPrefix(documented, "core provider remove"):
...
func onboardingKubeconfig(t *testing.T) string {
	...
	if path := os.Getenv("CORE_TEST_KUBECONFIG"); path != "" {
		...
		if err != nil {
			t.Fatalf("CORE_TEST_KUBECONFIG is unreadable: %v", err)
		}
		return string(body)
	}
