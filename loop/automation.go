...
func AccountStepCost(ctx context.Context, store uc.Store, run uc.AgentRun, step uc.RunStep) error {
	return store.Tx(ctx, func(txs uc.Store) error {
		...
		if err := scope.Memory().Set(ctx, uc.SessionMemoryEntry{SessionID: run.SessionID, Key: "system.cost.latest", Value: value, UpdatedBy: uc.Actor{Type: uc.ActorSystem}}); err != nil {
			return err
		}
		payload, _ := json.Marshal(uc.HookFiredPayload{Hook: "cost-accounting", RunID: run.ID})
		_, err := scope.Events().Append(ctx, run.SessionID, uc.Event{Actor: uc.Actor{Type: uc.ActorSystem}, Kind: uc.EventKindHookFired, Payload: payload})
		return err
	})
}
